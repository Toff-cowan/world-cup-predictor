...
import ForumCommentPreview from "../../components/forum/ForumCommentPreview.jsx";
        ...
        <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/70 mt-2 m-0 max-w-xl">
              Ask questions, share brackets, vote on picks, and copy them into your predictions.
            </p>
          </div>
          {isAuthenticated ? (
            <Link
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 py-8 sm:py-10">
        ...
        {!loading && posts.length > 0 && (
          <ul className="space-y-4 list-none m-0 p-0">
            {posts.map((post) => (
              <li key={post.id}>
                <article className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 overflow-hidden">
                  <Link
                    to={`/forum/${post.id}`}
                    state={{ preview: post }}
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      {post.share_token ? (
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 shrink-0">
                          Question
                        </span>
                      )}
                    </div>
